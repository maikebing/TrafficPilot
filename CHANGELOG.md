...
- **在线更新**：在 `About` 页面一键检查并下载 GitHub / Gitee 最新版本，程序自动替换并重启。
- **快捷配置切换**：最近使用的配置文件以快捷按钮形式呈现，同步显示在托盘菜单。
- **开机启动 & 自动启动代理**：支持注册用户级启动项，及程序加载配置后自动启动代理。

### 文档更新
- `README.md` 全面重写：补充域名路由、DoH 筛选、自动刷新、本机代理、配置云同步等所有新功能的说明，更新配置示例与配置项表格，修正默认代理协议（`socks5`），更新技术架构组件表。
- `About` 页描述重写：使用中文全面列出当前版本所有功能特性。
