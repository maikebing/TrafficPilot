...
- **GitHub520 Hosts 重定向**：自动下载 [GitHub520](https://github.com/521xueweihan/GitHub520) 维护的 hosts 列表，通过 DNS 拦截将域名解析结果替换为对应 IP，使 GitHub 等网站可直接访问（无需代理）
...
2. 勾选 `Enable GitHub520 Hosts Redirect`
...
| `hostsRedirect.enabled` | bool | 是否启用 GitHub520 Hosts 重定向 |
...
5. **DNS 拦截**（Hosts Redirect 功能）：通过独立的 WinDivert 句柄拦截入站 UDP 53 端口的 DNS 响应报文，解析其中的 A 记录，若域名匹配 GitHub520 hosts 列表则将 IP 替换为对应值后重新注入，使应用程序直接连接到 GitHub520 提供的 IP 地址
